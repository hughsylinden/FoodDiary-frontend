...
function LandingPage() {  
  ...
  return (
    <div>      
      ...
      { 
        showDiaryInputs && 
          <div>diary name<input type="text" name="name" onChange={handleFieldChange}/>      
          <button onClick={handleAddFoodDiary}>
            add  
          </button><br/>        
        </div>
      }

      <button onClick={viewNewDiaryInputs}>create new food diary</button><br/><br/>
      {
        foodDiary.map(fd=>
          <Link
              to={`/FoodDiary/${fd.id}`}
